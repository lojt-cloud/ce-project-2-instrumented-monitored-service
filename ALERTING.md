...
`AuthService-AccountLockout-Critical`: treat as confirmed evidence of an active or very recent brute-force attempt. Query Logs Insights for `login_failed` and `account_locked` events for the affected username to get the source `ip` and a timeline. The app has already protected the account by locking it, no immediate action is required there, but the `ip` is worth recording for any follow-up.
